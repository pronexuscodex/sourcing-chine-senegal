...
export default function QuotesListPage() {
  ...
  const { data: quotes, isLoading } = useQuery({
    ...
    queryFn: () => authFetch((token) => api.get<QuoteView[]>('/quotes/mine', token)),
  });
    ...
    <div className="flex flex-col gap-6">
      ...
      {isLoading && (
        <div className="flex justify-center py-12">
          <Spinner label="Chargement de vos devis…" />
        </div>
      )}
      ...
      {!isLoading && quotes?.length === 0 && (
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-gray-300 py-16 text-center">
          ...
          <p className="text-sm text-gray-500">Vous n&apos;avez pas encore reçu de devis.</p>
        </div>
      )}
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {quote.items[0]?.requestItem.description ?? 'Produit sans description'}
                    {quote.items.length > 1 ? ` et ${quote.items.length - 1} autre(s)` : ''}
                  </p>
                  <p className="mt-1 text-xs text-gray-400">Valable jusqu&apos;au {formatDate(quote.validUntil)}</p>
                </div>
                <div className="flex items-center gap-2">
